...
def main(client, adgroup_id):
  """Runs the example."""
  adgroup_criterion_service = client.GetAdGroupCriterionService(
      version='v201402')
  ...
  result = adgroup_criterion_service.Mutate(helper.GetOperations())[0]
...
def DisplayTree(node, children, level=0):
  ...
  if 'caseValue' in node:
    type = node['caseValue']['ProductDimension_Type']

    if type == 'ProductCanonicalCondition' and 'condition' in node['caseValue']:
      value = node['caseValue']['condition']
    elif type == 'ProductBiddingCategory':
      value = '%s(%s)' % (node['caseValue']['type'], node['caseValue']['value']
                          if 'value' in node['caseValue'] else 'OTHER')

    else:
      if 'condition' in node['caseValue']:
        value = node['caseValue']['condition']

  print '%sid: %s, type: %s, condition: %s\n' % (' ' * level, node['id'], type,
                                                 value)
