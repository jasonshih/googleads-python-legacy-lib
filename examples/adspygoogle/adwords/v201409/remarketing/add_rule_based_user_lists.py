...
def main(client):
  ...
  adwords_user_list_service = client.GetAdwordsUserListService(
      version='v201409')
  ...
  user_lists = adwords_user_list_service.mutate(operations)[0]

  # Display results.
  for user_list in user_lists['value']:
    print (('User list added with ID %s, name "%s", status "%s", list type'
