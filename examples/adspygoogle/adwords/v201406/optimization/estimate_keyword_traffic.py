...
def main(client):
  ...
  for index in xrange(len(keyword_estimates)):
    ...
    if ('averageCpc' in min and 'averagePosition' in min
        and 'averageCpc' in max and 'averagePosition' in max):
      # Find the mean of the min and max values.
      mean_avg_cpc = (long(min['averageCpc']['microAmount']) +
                      long(max['averageCpc']['microAmount'])) / 2
      mean_avg_pos = (float(min['averagePosition']) +
                      float(max['averagePosition'])) / 2
      mean_clicks = (float(min['clicksPerDay']) +
                     float(max['clicksPerDay'])) / 2
      mean_total_cost = (long(min['totalCost']['microAmount']) +
                         long(max['totalCost']['microAmount'])) / 2
      ...
      print '  Insufficient data to calculate estimates.'
